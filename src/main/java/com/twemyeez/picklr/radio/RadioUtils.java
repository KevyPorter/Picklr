...
public class RadioUtils {
	...
	public static void rawplay(AudioFormat targetFormat,
			AudioInputStream decodedInput) throws IOException,
			LineUnavailableException {
		byte[] data = new byte[4096];

		// Create the source data line
		SourceDataLine sourceDataLine = (SourceDataLine) AudioSystem
				.getLine(new DataLine.Info(SourceDataLine.class, targetFormat));
	...
	public static void startRadio() {
		...
		if (!inProgress) {
			...
			CommonUtils.sendFormattedChat(true, "HypixelRadio.net toggled on",
					EnumChatFormatting.GREEN, true);

			// Start a new Radio thread to do data retrieval and playing in the
			// background
	...
	public static void stopRadio() {
		...
		if (inProgress) {
			...
			CommonUtils.sendFormattedChat(true, "HypixelRadio.net toggled off",
			...
			new Thread(new Runnable() {
				...
				@Override
				public void run() {
					// the difference from -70F to -80F (the minimum volume) is
					// virtually indistinguishable, hence we use -70
					...
					thread.stop();
					// the thread has stopped, so let's reset the Boolean
					// inProgress flag to false to indicate the radio can be
					// started again
					inProgress = false;
				}
	...
	/*
	 * This handles the simple decision between whether the radio is on or not,
	 * and acts accordingly, toggling the state
	 */
