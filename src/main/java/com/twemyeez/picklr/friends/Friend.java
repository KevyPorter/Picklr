...
public class Friend {
	/*
	 * This object represents a friend, and contains data on their skin,
	...
	// Username of the friend with any formatting from the friends list, such as
	// rank colour
	...
	// This boolean indicates whether the texture is loaded, or we have to load
	// it
	 ...
	 * Friend constructor
	 */
	public Friend(final String username, String formattedUsername,
			String location) {
	...
	public void drawHead(int headX, int headY, int headWidth, int headHeight) {
		...
		if (!textureLoaded) {
			try {
				...
				skinResourceLocation = skinTextureManager
						.getDynamicTextureLocation("skinTexture", skinTexture);

				// Indicate we've now loaded the skin, so we don't reload it in
				// future
				textureLoaded = true;
			} catch (IOException e) {
				// If an error occurred, we can print the stack trace for
				// debugging
				e.printStackTrace();
			}
		}
		...
		tessellator.addVertexWithUV(headX + headWidth, headY + headHeight, 0,
				1.0, 1.0);
