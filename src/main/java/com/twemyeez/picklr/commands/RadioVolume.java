...
public class RadioVolume implements ICommand{
	...
	public void processCommand(ICommandSender sender, String[] args) {
		...
		else
		{
			...
			{
				//If there has been an exception, it's probably in the integer parsing, so we'll return a syntax hint message
				e.printStackTrace();CommonUtils.sendFormattedChat(true, "Radio volume adjustment is done via /volume [percentage]", EnumChatFormatting.RED, true);
			}
		}
	}

	//There are no required limitations on who can use the command, so we'll simply return true
	@Override
	public boolean canCommandSenderUseCommand(ICommandSender p_71519_1_) {
		return true;
	}

	//This takes a percentage, so tab complete really won't be that useful, so simply return null
	@Override
