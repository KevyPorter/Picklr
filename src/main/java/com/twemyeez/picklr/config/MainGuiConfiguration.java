...
public class MainGuiConfiguration extends GuiConfig {
	...
	public MainGuiConfiguration(GuiScreen parent) {
		...
		super(parent, new ConfigElement(ConfigurationHandler.getMainConfig()
				.getCategory(Configuration.CATEGORY_GENERAL))
				.getChildElements(), Picklr.MODID, // Register the config for
													// the correct mod ID
				false, // define no world restart needed
				false, // define no MC restart required
				GuiConfig.getAbridgedConfigPath(ConfigurationHandler
						.getMainConfig().toString()));
	}
